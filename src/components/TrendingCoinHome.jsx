  ...
  useEffect(() => {
    const fetchTrendingCoins = async () => {
      try {
        ...
        setTrendingCoins(data.coins.slice(0, 5)); // Only display data of 5 coins
        setLoading(false);
      } catch (error) {
...
return (
    <>
            ...
            <table className="table-auto w-full">
              ...
              <tbody className='text-white'>
                {trendingCoins.slice(0, 5).map((coin) => (
                  <tr key={coin.item.id} className='hover:bg-slate-900'>
      ...
      {/* <TrendingCoinsTable trendingCoins={trendingCoins} numberOfCoins={5}/> */}
    </>
  );
};

export default TrendingCoinsHome;